...
public interface BookingRepository extends JpaRepository<Booking, Long> {
    ...
    /**
     * An existing pending attempt for this user+slot — payment retries reuse it
     * instead of duplicating.
     */
    Optional<Booking> findBySlotIdAndUserIdAndStatus(Long slotId, Long userId, BookingStatus status);

    /**
     * Other live bookings on the same slot — used to decide whether cancelling may
     * release it.
     */
    ...
    List<Booking> findByStatusAndBookingDate(BookingStatus status, java.time.LocalDate bookingDate);

    /**
     * Distinct bookers active on or after a date — the numerator of the return
     * rate.
     */
