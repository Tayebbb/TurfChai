...
public interface SlotRepository extends JpaRepository<Slot, Long> {
    ...
    List<Slot> findByStatusAndHoldExpiresAtBefore(
        SlotStatus status,
     ...
     */
    @Query(
        "SELECT s FROM Slot s LEFT JOIN FETCH s.pitch WHERE s.heldByUserId = :heldByUserId " +
     ...
     */
    @Query(
        "SELECT s FROM Slot s JOIN FETCH s.pitch WHERE s.venueId = :venueId AND s.slotDate = :slotDate " +
            "ORDER BY s.startTime ASC"
    )
    List<Slot> findByVenueIdAndSlotDateOrderByStartTimeAsc(
    ...
    boolean existsByPitchIdAndSlotDateAndStartTime(
        Long pitchId,
        LocalDate slotDate,
        LocalTime startTime
    );
    ...
    /** @deprecated duplicates are possible (seeders + SQL migrations); prefer {@link #findAllByPitchIdAndSlotDateAndStartTime}. */
    @Deprecated
    Optional<Slot> findByPitchIdAndSlotDateAndStartTime(
        Long pitchId,
        LocalDate slotDate,
        LocalTime startTime
    );

    List<Slot> findByVenueIdAndSlotDateBetweenOrderBySlotDateAscStartTimeAsc(
        Long venueId,
        LocalDate startDate,
        LocalDate endDate
    );
    ...
    List<Slot> findByVenueIdInAndSlotDateBetween(
        List<Long> venueIds,
        LocalDate startDate,
        LocalDate endDate
    );
    ...
    @Query(
        "SELECT s FROM Slot s JOIN FETCH s.pitch WHERE s.venueId = :venueId AND (s.slotDate > :today OR (s.slotDate = :today AND s.startTime >= :now)) AND s.status = com.turfchai.booking.entity.SlotStatus.AVAILABLE"
    )
    List<Slot> findUpcomingAvailableSlots(
        @Param("venueId") Long venueId,
