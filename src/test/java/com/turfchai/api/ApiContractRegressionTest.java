...
class ApiContractRegressionTest {
        ...
        /**
         * The join body used to require a {@code userId} the service deliberately
         * ignores, so the honest request the UI sends was rejected as invalid and
         * nobody could join a game at all.
         */
        @Test
        @DisplayName("Joining an open game does not demand a user id the server ignores")
        void joinOpenGameTakesThePlayerFromTheToken() throws Exception {
                String created = mvc.perform(post("/api/v1/solo/open-games")
                                .header(HttpHeaders.AUTHORIZATION, adminToken)
                                .contentType(MediaType.APPLICATION_JSON)
                                .content("""
                                                {"title":"Contract game","venueId":%d,"gameDate":"%s",
                                                 "startTime":"21:00:00","endTime":"22:30:00",
                                                 "skillLevel":"ALL_LEVELS","capacity":4,
                                                 "pricePerPlayer":250,"minimumReliability":0}
                                                """.formatted(venueId, LocalDate.now().plusDays(3))))
                                .andExpect(status().isCreated())
                                .andReturn().getResponse().getContentAsString();
                long gameId = Long.parseLong(created.replaceAll("(?s).*?\"id\"\\s*:\\s*(\\d+).*", "$1"));

                mvc.perform(post("/api/v1/solo/open-games/" + gameId + "/join")
                                .header(HttpHeaders.AUTHORIZATION, playerToken)
                                .contentType(MediaType.APPLICATION_JSON)
                                .content("{\"paymentMethod\":\"bKash\"}"))
                                .andExpect(status().isOk())
                                .andExpect(jsonPath("$.filledCount").value(2));
        }

        /** Out-of-range input is the caller's mistake; it used to persist and 500. */
        @Test
        @DisplayName("A reliability bar above the maximum is a bad request, not a server error")
        void impossibleReliabilityBarIsRejected() throws Exception {
                mvc.perform(post("/api/v1/solo/open-games")
                                .header(HttpHeaders.AUTHORIZATION, playerToken)
                                .contentType(MediaType.APPLICATION_JSON)
                                .content("""
                                                {"title":"Elite only","venueId":%d,"gameDate":"%s",
                                                 "startTime":"07:00:00","endTime":"08:00:00",
                                                 "skillLevel":"ALL_LEVELS","capacity":4,
                                                 "pricePerPlayer":300,"minimumReliability":200}
                                                """.formatted(venueId, LocalDate.now().plusDays(3))))
                                .andExpect(status().isBadRequest());
        }

        // ── Turf verification requests ──────────────────────────────────────────

        /**
         * A verification request used to be padded out on the server: a blank
         * phone became +8801811223344, and the three document fields became
         * "Trade_License.pdf", "NID.pdf" and "Utility_Bill.pdf" — so the admin
         * reviewed a request that looked fully documented when nothing had been
         * supplied.
         */
        @Test
        @DisplayName("A turf request keeps only what the owner actually supplied")
        void turfRequestDoesNotInventMissingDetails() throws Exception {
                mvc.perform(post("/api/v1/turf-requests")
                                .header(HttpHeaders.AUTHORIZATION, playerToken)
                                .contentType(MediaType.APPLICATION_JSON)
                                .content("{\"venueName\":\"Bare Venue\",\"area\":\"Dhanmondi\"}"))
                                .andExpect(status().isOk())
                                .andExpect(jsonPath("$.venueName").value("Bare Venue"))
                                .andExpect(jsonPath("$.ownerPhone").doesNotExist())
                                .andExpect(jsonPath("$.sportsCsv").doesNotExist())
                                .andExpect(jsonPath("$.docTradeLicense").value("PENDING"))
                                .andExpect(jsonPath("$.docOwnerNid").value("PENDING"))
                                .andExpect(jsonPath("$.docUtilityBill").value("PENDING"));
        }
}
