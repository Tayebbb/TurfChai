 ...
 *
 * <p>Every assertion here corresponds to a vulnerability that was reproduced
 ...
 *
 * <p>Covered findings: TC-001 (header impersonation), TC-002 (tournament
    ...
    void bookingsAreNotReadableAcrossUsers() throws Exception {
        mvc.perform(get("/api/v1/bookings/999999").header(HttpHeaders.AUTHORIZATION, auth(playerB)))
                .andExpect(result -> {
                    ...
                    if (s == 200) {
                        throw new AssertionError("A stranger must never receive another user's booking");
                    }
                });
        mvc.perform(get("/api/v1/payments/booking/999999").header(HttpHeaders.AUTHORIZATION, auth(playerB)))
                .andExpect(result -> {
                    int s = result.getResponse().getStatus();
                    if (s == 200) {
                        throw new AssertionError("A stranger must never receive another user's payments");
                    }
                });
    }
    ...
    void secretsAreNeverSerialised() throws Exception {
        ...
        for (String body : new String[]{profile, me, adminUsers}) {
            if (body.contains("passwordHash") || body.contains("password_hash")
                    || body.contains("twoFactorSecret") || body.contains("$2a$") || body.contains("$2b$")) {
                throw new AssertionError("Response leaked a credential: " + body.substring(0, Math.min(200, body.length())));
            }
        }
    }
